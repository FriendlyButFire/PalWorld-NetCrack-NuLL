...
namespace DX11_Base
{
	...
	namespace Tabs
	{
		...
		void TABExploit()
		{
			...
			ImGui::Checkbox("Open MapObject List", &Config.bisOpenMapObj);
		...
		void TABMisc()
		{
			...
			if (ImGui::Button("Heal Aura", ImVec2(ImGui::GetContentRegionAvail().x - 3, 20)))
			{
				if (Config.GetPalPlayerCharacter() != NULL)
				{
					if (Config.GetPalPlayerCharacter()->GetPalPlayerController() != NULL)
							...
							for (int i = 0; i < T.Count(); i++)
							{
								...
								if (T[i] != NULL)
								{
									if (T[i]->IsA(SDK::APalCharacter::StaticClass()))
									{
										...
										SDK::UPalCharacterParameterComponent* pParams = monster->CharacterParameterComponent;
										if (!pParams)
											return;
										...
										SDK::EPalBaseCampWorkerSickType sick = ivParams->GetWorkerSick();

										Config.GetPalPlayerCharacter()->GetPalPlayerController()->Transmitter->NetworkIndividualComponent->SetWorkerSick_ToServer(monster->CharacterParameterComponent->IndividualHandle->ID, SDK::EPalBaseCampWorkerSickType::None);

										monster->CharacterParameterComponent->IndividualParameter->SetPhysicalHealth(SDK::EPalStatusPhysicalHealthType::Healthful);
			...
			for (int i = 0; i < T.Count(); i++)
			{
				...
				*/
				...
				if (ImGui::Button("Heal"))
				{
					...
					Config.GetPalPlayerCharacter()->GetPalPlayerController()->Transmitter->NetworkIndividualComponent->SetWorkerSick_ToServer(Character->CharacterParameterComponent->IndividualHandle->ID, SDK::EPalBaseCampWorkerSickType::None);

					Character->ReviveCharacter_ToServer(SDK::FFixedPoint(100));

					SDK::EPalStatusHungerType hunger = ivParams->GetHungerType();

					//Character->CharacterParameterComponent->IndividualParameter->UpdateHungerTypeDelegate__DelegateSignature(SDK::EPalStatusHungerType::Default, hunger);
					
					Character->CharacterParameterComponent->IndividualParameter->SetPhysicalHealth(SDK::EPalStatusPhysicalHealthType::Healthful);
				}
				ImGui::SameLine();
				if (ImGui::Button("DORMI"))
				{
					SDK::UPalUtility* aPalUtility = SDK::UPalUtility::GetDefaultObj();

					if (!aPalUtility) return; 
					static SDK::UKismetStringLibrary* lib = SDK::UKismetStringLibrary::GetDefaultObj();

					std::string statusname = "Sleep";
					wchar_t  ws[255];
					swprintf(ws, 255, L"%hs", statusname.c_str());
					SDK::FName statusName = lib->Conv_StringToName(SDK::FString(ws));

					aPalUtility->SetStatusPoint(Character->CharacterParameterComponent->IndividualHandle, statusName, 999999);
				}
				...
				if (ImGui::Button("capture"))
				{
					if (T[i]->IsA(SDK::APalCharacter::StaticClass()))
					{
						if (Character)
						{
							...
							if (!config::GetAllActorsofType(SDK::APalCaptureJudgeObject::StaticClass(), &actors, true))
								return;

							for (auto actor : actors)
							{
								SDK::APalCaptureJudgeObject* judge = static_cast<SDK::APalCaptureJudgeObject*>(actor); // Try to cast actor to APalCapturedCage
								if (!judge)
									continue; // Skip to the next actor if casting fails

								judge->ChallengeCapture_ToServer(Character, 1.f);
								judge->ChallengeCapture(Character, 1.f);
							}
						}
					}
				}
				...
				if (Character->IsA(SDK::APalPlayerCharacter::StaticClass()))
				{
					...
					if (ImGui::Button("Boss"))
					{
						if (Config.GetPalPlayerCharacter() != NULL)
						{
							...
							if (controller != NULL)
							{
								controller->Transmitter->BossBattle->RequestBossBattleEntry_ToServer(SDK::EPalBossType::SnowBoss, (SDK::APalPlayerCharacter*)Character);
								controller->Transmitter->BossBattle->RequestBossBattleStart_ToServer(SDK::EPalBossType::SnowBoss, (SDK::APalPlayerCharacter*)Character);
							}
						}
					}
					ImGui::SameLine();
					if (ImGui::Button("Add Super Buff"))
					{
						if (Config.GetPalPlayerCharacter() != NULL)
						{
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::AttackUp);
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::ControlSP);
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::DefenseUp);
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::LifeSteal);
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::Resistance);
							Config.GetPalPlayerCharacter()->StatusComponent->AddStatus(SDK::EPalStatusID::ShieldRecovery);
						}
					}
					ImGui::SameLine();
					if (ImGui::Button("Remove Super Buff"))
					{
						if (Config.GetPalPlayerCharacter() != NULL)
						{
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::AttackUp);
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::ControlSP);
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::DefenseUp);
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::LifeSteal);
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::Resistance);
							Config.GetPalPlayerCharacter()->StatusComponent->RemoveStatus(SDK::EPalStatusID::ShieldRecovery);
						}
					}
				}
				ImGui::PopID();
	...
	void Menu::GuildList()
	{
		if (!ImGui::Begin("MapObject List", &g_GameVariables->m_ShowMenu, 96))
		{
			ImGui::End();
			return;
		}
		...
		if (Config.gWorld)
		{
			...
			static char map_search[100];

			ImGui::InputText("Search", map_search, IM_ARRAYSIZE(map_search));
			ImGui::BeginChild("ScrollingRegion", ImVec2(500, 500), true);

			SDK::TArray<SDK::AActor*> T = Config.GetUWorld()->PersistentLevel->Actors;
			SDK::APalMapObject* MapObject{};
			SDK::APalGuildInfo* GInfo;
			std::string name;
			SDK::UPalUtility* aPalUtility = SDK::UPalUtility::GetDefaultObj();
			...
			for (int i = 0; i < T.Count(); i++)
			{
				...
				if (!T[i]->IsA(SDK::APalMapObject::StaticClass()))
					continue;

				//if (!T[i]->IsA(SDK::APalGuildInfo::StaticClass()))
				//	continue;
				...
				if (T[i]->IsA(SDK::APalGuildInfo::StaticClass())) {
					...
					SDK::FString guildName = SDK::FString();

					aPalUtility->GetGroupManager(Config.GetUWorld())->TryGetGuildName(static_cast<SDK::APalGuildInfo*>(T[i])->GroupID, &guildName);

					name = guildName.ToString();
				}

				auto name_to_lower = name;
				std::string map_search_to_lower = map_search;

				std::transform(name_to_lower.begin(), name_to_lower.end(), name_to_lower.begin(), ::tolower);
				std::transform(map_search_to_lower.begin(), map_search_to_lower.end(), map_search_to_lower.begin(), ::tolower);

				if (map_search[0] != '\0' && (name_to_lower.find(map_search_to_lower) == std::string::npos))
					continue;


				std::regex pattern("BP_MapObject_|BP_BuildObject_");


				// Replace "BP_MapObject_" with an empty string
				std::string name_cleared = std::regex_replace(name, pattern, "");


				ImGui::Text(name_cleared.c_str());
				ImGui::SameLine();
				ImGui::PushID(i);
				if (ImGui::Button("Kill"))
				{
					SDK::FPalDamageInfo  info = SDK::FPalDamageInfo();
					info.AttackElementType = SDK::EPalElementType::Fire;
					info.Attacker = Config.GetPalPlayerCharacter();
					info.AttackerGroupID = Config.GetPalPlayerState()->IndividualHandleId.PlayerUId;
					info.AttackerLevel = 50;
					info.AttackType = SDK::EPalAttackType::Weapon;
					...
					if (T[i]->IsA(SDK::APalMapObject::StaticClass())) {
						...
						MapObject = static_cast<SDK::APalMapObject*>(T[i]);
						Config.GetPalPlayerCharacter()->GetPalPlayerController()->Transmitter->MapObject->RequestDamageMapObject_ToServer(MapObject->ModelInstanceId, info);
					}
				}
				ImGui::SameLine();
				if (ImGui::Button("TP"))
				{
					...
					SDK::FVector vector = T[i]->K2_GetActorLocation();
					AnyWhereTP(vector, Config.IsSafe);
				...
				if (ImGui::Button("info")) {
					...
					if (T[i]->IsA(SDK::APalGuildInfo::StaticClass())) {
						GInfo = static_cast<SDK::APalGuildInfo*>(T[i]);

						data = GInfo->Guild->GuildName.ToString();
					}

					if (T[i]->IsA(SDK::APalMapObject::StaticClass())) {
						MapObject = static_cast<SDK::APalMapObject*>(T[i]);

						data = MapObject->MapObjectModel->BuildObjectId.ToString();

					}

					g_Console->printdbg("\n\n[+] Data: %s\n", Console::Colors::green, data);
				}
				ImGui::PopID();
